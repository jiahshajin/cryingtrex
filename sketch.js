...
function draw() {
 // background("white");
  ...
  textSize(23);
  textFont("caligraphy");
  text("Score: "+ score, width/6,width/69);
  text("HI= "+ localServer[0],width/20,height/69)
  ...
  if(gameState === PLAY){
     //Making trex jump when pressing space or touching the screen.
      if(  keyDown("space")   && trex.y >= height- 180|| touches.length > 0  ){
        trex.velocityY = -10;
        jumpSound.play();
        touches = [];
    } 
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(890  ,height-260,10,40);    
